...
// CRUD de formatos de objeto (campos parametrizables) y de los objetos master del catálogo.
// Si se pasa systemId, los formatos creados quedan ligados a ese sistema de juego.
export default function ItemsPanel({ user, systemId = null }) {
  const [formats, setFormats] = useState([]);
  const [selected, setSelected] = useState(null);
  const [newFormatName, setNewFormatName] = useState('');
  const [newFieldName, setNewFieldName] = useState('');
  const [newItemName, setNewItemName] = useState('');
  const [newItemEquippable, setNewItemEquippable] = useState(true);
  const [newItemValues, setNewItemValues] = useState({});
  const [error, setError] = useState('');

  async function loadFormats() {
    try {
      const { formats: list } = await api.listItemFormats(user.id, systemId);
      setFormats(list);
    } catch (err) {
  ...
  async function createFormat(e) {
    ...
    setError('');
    try {
      await api.createItemFormat(user.id, newFormatName.trim(), systemId);
      setNewFormatName('');
      await loadFormats();
    } catch (err) {
      setError(err.message);
  ...
  async function addField(e) {
    ...
    setError('');
    try {
      await api.createItemField(selected.id, user.id, {
  ...
  async function addItem(e) {
    ...
    setError('');
    try {
      await api.createItem(user.id, selected.id, newItemName.trim(), '', newItemEquippable, newItemValues);
      setNewItemName('');
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card className="p-4">
        <h3 className="mb-3 text-sm font-semibold text-gray-200">Nuevo formato de objeto</h3>
        <form onSubmit={createFormat} className="flex flex-col gap-3 md:flex-row">
          <input
            value={newFormatName}
            onChange={(e) => setNewFormatName(e.target.value)}
            placeholder="Nombre del formato (p. ej. Armas)"
      ...
      <section className="flex flex-col gap-3">
        {formats.length === 0 ? (
          <p className="text-center text-sm text-gray-500">Aún no hay formatos de objeto.</p>
        ) : (
          formats.map((fmt) => (
            <Card key={fmt.id} className="p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-gray-400">{fmt.fields.length} campos</p>
                </div>
                  ...
                  <div>
                      ...
                      {selected.fields.map((f) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              api
                                .deleteItemField(selected.id, f.id, user.id)
                    ...
                    <form onSubmit={addField} className="flex gap-2">
                      <input
                        ...
                        placeholder="Nuevo campo (p. ej. damage)"
                        className={`flex-1 ${inputCls}`}
                      />
                  ...
                  <div>
                    ...
                    <div className="mb-3 flex flex-col gap-2">
                      {selected.items.map((it) => (
                        ...
                        >
                          <div className="min-w-0">
                            <span className="text-sm text-gray-100">{it.name}</span>
                            {!it.equippable && (
                              <span className="ml-2 text-xs text-gray-500">(no equipable)</span>
                            )}
                            <span className="ml-2 text-xs text-gray-500">
                              {selected.fields
                                .map((f) => it.values[f.id])
                                .filter(Boolean)
                                .join(' · ')}
                            </span>
                          </div>
                    ...
                    <form onSubmit={addItem} className="flex flex-col gap-2">
                      ...
                      {selected.fields.map((f) => (
                        <input
                          key={f.id}
                          value={newItemValues[f.id] ?? ''}
                          onChange={(e) =>
                            setNewItemValues((prev) => ({ ...prev, [f.id]: e.target.value }))
                          }
